        ...
        <main class="flex-1 overflow-y-auto bg-gray-50">
            <div class="p-8 md:p-12">
                <div class="flex justify-between items-center mb-10">
                    <div>
                        <h2 class="text-3xl font-black text-gray-800 uppercase tracking-tight">Verifikasi UMKM</h2>
                        <p class="text-gray-400 text-sm font-medium mt-1">Setujui pendaftaran mitra UMKM baru di
                            platform MenuGO</p>
                    </div>
                </div>
                        ...
                        <table class="w-full text-left border-collapse">
                            <thead>
                                <tr class="bg-gray-50/50 border-b border-gray-100">
                                    <th class="px-6 py-5 text-xs font-bold uppercase text-gray-400 tracking-widest">
                                        Detail UMKM</th>
                                    <th class="px-6 py-5 text-xs font-bold uppercase text-gray-400 tracking-widest">
                                        Pemilik</th>
                                    <th class="px-6 py-5 text-xs font-bold uppercase text-gray-400 tracking-widest">
                                        Alamat Toko</th>
                                    <th
                                        class="px-6 py-5 text-xs font-bold uppercase text-gray-400 tracking-widest text-center">
                                        Aksi</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-50">
                                @forelse($pendingUmkms as $umkm)
                                    <tr class="hover:bg-gray-50/80 transition-all duration-200">
                                        ...
                                        <td class="px-6 py-5 text-gray-600 font-bold italic italic italic italic">
                                            {{ $umkm->user->name }}</td>
                                        <td
                                            class="px-6 py-5 text-gray-500 font-medium text-sm leading-relaxed max-w-xs">
                                            {{ $umkm->alamat }}</td>
                                        <td class="px-6 py-5">
                                            <div class="flex items-center justify-center gap-3">
                                                <form action="{{ route('admin.verifikasi.approve', $umkm) }}"
                                                    method="POST">
                                                    @csrf
                                                    <button type="submit"
                                                        class="bg-green-600 text-white px-5 py-2.5 rounded-xl text-[10px] font-black uppercase hover:bg-green-700 transition shadow-lg shadow-green-100">
                                                        Setujui
                                                    </button>
                                                </form>

                                                <form action="{{ route('admin.verifikasi.reject', $umkm) }}"
                                                    method="POST">
                                                    @csrf
                                                    <button type="submit"
                                                        class="bg-red-50 text-red-600 px-5 py-2.5 rounded-xl text-[10px] font-black uppercase hover:bg-red-600 hover:text-white transition">
                                                        Tolak
                                                    </button>
                                                </form>
                                            </div>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="px-6 py-20 text-center">
                                            <div
                                                class="w-20 h-20 bg-gray-50 rounded-3xl flex items-center justify-center mx-auto mb-4 text-gray-300">
                                            ...
                                            <p class="text-gray-400 font-bold uppercase text-[10px] tracking-widest">
                                                Tidak ada antrian verifikasi</p>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
